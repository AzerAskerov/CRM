...
{
    public partial class CargoPrintOut
    {
        ...
        protected async override Task OnParametersSetAsync()
        {
            ...
            var responseCargo = await HttpClient.PostAsJsonAsync("api/Deal/GetCargoDataInsurance",
                              new DealOfferDropdownDataModel
                              {
                                 PackagingId = OfferModel.Packaging,
                                 TransportationId = OfferModel.TransportationKind,
                              }
                              );


            var responseResultCargo = await responseCargo?.Content?.ReadFromJsonAsync<OperationResult<DealOfferDropdownDataModel>>();

            OfferModel.PackagingName = responseResultCargo?.Model?.PackagingName;
            OfferModel.TransportationKindName = responseResultCargo?.Model?.TransportationName;
