...
{
    public class VoluntaryHealthEmployeeTypeSourceLoader : SourceLoader
    {
        public VoluntaryHealthEmployeeTypeSourceLoader()
        {
        }
        ...
        protected override IEnumerable<ISourceItem> DoLoad2(IModel model)
        {
            ...
            foreach (var code in context.Codes.Where(x =>
                x.TypeOid == (int) CodeTypeEnum.VOLUNTARY_HEALTH_EMPLOYEE_TYPE && DateTime.Now > x.ValidFrom && DateTime.Now < x.ValidTo))
            {
