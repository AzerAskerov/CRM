...
{
    public class OfferPolicyPeriodSourceLoader : SourceLoader
    {
        public OfferPolicyPeriodSourceLoader()
        {
        }
        ...
        protected override IEnumerable<ISourceItem> DoLoad2(IModel model)
        {
            ...
            foreach (var code in context.Codes.Where(x =>
                x.TypeOid == (int) CodeTypeEnum.OFFER_POLICY_PERIOD && DateTime.Now > x.ValidFrom && DateTime.Now < x.ValidTo))
            {
                list.Add(new SourceItem() {Key = code.CodeOid, Value = code.Value.Translate()});
            }
        ...
        public ISourceItem GetItem(object key, CRMDbContext context = null)
        {
            ...
            var item = context.Codes.FirstOrDefault(x => x.CodeOid == int.Parse(key.ToString()) && x.TypeOid == (int)CodeTypeEnum.OFFER_POLICY_PERIOD && DateTime.Now > x.ValidFrom &&
