...
{
    public class VoluntaryHealthEmployeeAgeRangeSourceLoader : SourceLoader
    {
        public VoluntaryHealthEmployeeAgeRangeSourceLoader()
        {
        }
        ...
        protected override IEnumerable<ISourceItem> DoLoad2(IModel model)
        {
            ...
            foreach (var code in context.Codes.Where(x =>
                x.TypeOid == (int) CodeTypeEnum.VOLUNTARY_HEALTH_EMPLOYEE_AGE_RANGES && DateTime.Now > x.ValidFrom && DateTime.Now < x.ValidTo))
            {
