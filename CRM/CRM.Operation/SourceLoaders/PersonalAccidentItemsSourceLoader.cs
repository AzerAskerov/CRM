...
{
    public class PersonalAccidentItemsSourceLoader : SourceLoader
    {
        public PersonalAccidentItemsSourceLoader()
        {
        }
        ...
        protected override IEnumerable<ISourceItem> DoLoad2(IModel model)
        {
            ...
            foreach (var code in context.Codes.Where(x =>
                x.TypeOid == (int) CodeTypeEnum.PERSONAL_ACCIDENT_ITEMS && DateTime.Now > x.ValidFrom && DateTime.Now < x.ValidTo).ToList().OrderBy(x=>decimal.Parse(x.Value)))
            {
        ...
        public ISourceItem GetItem(object key, CRMDbContext context = null)
        {
            ...
            var item = context.Codes.FirstOrDefault(x => x.CodeOid == int.Parse(key.ToString()) && x.TypeOid == (int)CodeTypeEnum.PERSONAL_ACCIDENT_ITEMS && DateTime.Now > x.ValidFrom &&
