...
namespace CRM.Operation.SourceLoaders
{
    ...
    public class TransportationSourceLoader : SourceLoader
    {
        public TransportationSourceLoader()
        {
        }
        protected override IEnumerable<ISourceItem> DoLoad2(IModel model)
        {
            ...
            foreach (var code in context.Codes.Where(x => x.TypeOid == (int)CodeTypeEnum.TRANSPORTATION_TYPE && DateTime.Now > x.ValidFrom && DateTime.Now < x.ValidTo))
            {
        ...
        public ISourceItem GetItem(object key, CRMDbContext context = null)
        {
            ...
            var item = context.Codes.FirstOrDefault(x => x.CodeOid == int.Parse(key.ToString()) && x.TypeOid == (int)CodeTypeEnum.TRANSPORTATION_TYPE && DateTime.Now > x.ValidFrom &&
