...
{
    public class SurveyerTypeSourceLoader : SourceLoader
    {
        protected CRMDbContext DbContext { get; set; }
        public SurveyerTypeSourceLoader()
        {
        }
        protected override IEnumerable<ISourceItem> DoLoad2(IModel model)
        {
            ...
            foreach (var code in context.Codes.Where(x =>
                x.TypeOid == (int) CodeTypeEnum.SURVEYER_TYPE && DateTime.Now > x.ValidFrom && DateTime.Now < x.ValidTo))
            {
