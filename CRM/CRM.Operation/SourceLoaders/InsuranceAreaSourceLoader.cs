...
{
    class InsuranceAreaSourceLoader: SourceLoader
    {
        public InsuranceAreaSourceLoader()
        {
        }
        ...
        protected override IEnumerable<ISourceItem> DoLoad2(IModel model)
        {
            ...
            foreach (var code in context.Codes.Where(x =>
                x.TypeOid == (int)CodeTypeEnum.INSURANCE_AREA && DateTime.Now > x.ValidFrom && DateTime.Now < x.ValidTo))
            {
        ...
        public ISourceItem GetItem(object key, CRMDbContext context = null)
        {
            ...
            var item = context.Codes.FirstOrDefault(x => x.CodeOid == int.Parse(key.ToString()) && x.TypeOid == (int)CodeTypeEnum.INSURANCE_AREA && DateTime.Now > x.ValidFrom &&
